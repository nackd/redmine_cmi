...
3. Edit the following files:
   ...
   * db/fixtures/history_profiles_costs.csv
     Hourly cost of each profile (by year).
   * db/fixtures/history_user_profiles.csv
     Historic user profile evolution: which user had which profile when sometime?
...
7. Enable the permissions for your Roles
...
values are created by the rake task cmi:create_data, based on the configuration
