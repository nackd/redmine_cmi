...
namespace :test do
  ...
  task :functionals do
    Dir.chdir(redmine) do
      exit false unless system %q{rake test:plugins:functionals PLUGIN=redmine_cmi RAILS_ENV=test}
    end
  end
  ...
  task :units do
    Dir.chdir(redmine) do
      exit false unless system %q{rake test:plugins:units PLUGIN=redmine_cmi RAILS_ENV=test}
    end
  end
